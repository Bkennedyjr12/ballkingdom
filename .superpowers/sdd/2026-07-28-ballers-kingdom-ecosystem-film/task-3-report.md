...
The attempted local Brian narration generation used only the now-pinned, authorized Chatterbox runtime and authorized reference. It failed before WAV creation/validation because that runtime lacks the local `perth` Python module. Per the no-fallback rule, the animatic has an intentional original low music/room-tone bed but **no substitute narration**. Repair the pinned local environment, generate/validate the 70-second authorized narration master, then rerun `render_animatic.sh` before external or paid-scene review.
