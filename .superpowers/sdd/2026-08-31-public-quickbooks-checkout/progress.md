...
Whole-branch review round 2: review found 3 Important issues (service Customer/Invoice/timeout ambiguity was retryable; operative Task 7 retained an obsolete one-flag/18-Function release path; public scheduled invoice effects ran only under the legacy owner alias and could not recover safely without persisted email).

Whole-branch review round 2: Ruling: `PROVIDER_TIMEOUT`, `QBO_CUSTOMER_AMBIGUOUS`, and `QBO_INVOICE_AMBIGUOUS` are terminal for service Invoice creation because each may follow a committed Customer or Invoice mutation — cost if wrong: an operator must reconcile a false ambiguity manually, while automatic duplicate mutation remains prohibited. Unclassified demonstrably pre-commit failure remains retryable for compatibility.

Whole-branch review round 2: Ruling: a production-public background worker may complete `invoice_create` only from already-persisted realm/Invoice/Customer/order references plus one exact authoritative Accounting readback. It never reads the protected owner email to recover a public order. A missing reference, mismatched evidence, or email-dependent send becomes terminal manual review with a redacted alert — cost if wrong: a customer may require operator recovery after a crash, favoring no blind Customer/Invoice/email retry over availability.

Whole-branch review round 2: Ruling: `supportsWebhooks=true` describes reviewed provider-account capability, not active ingestion. Webhook ingestion remains separately gated, advisory-only, and subordinate to authoritative scheduled Accounting reconciliation — cost if wrong: capability evidence alone cannot be used as proof that events are being received.

Whole-branch review round 2: all 3 Important findings addressed with no open finding. Node 22 verification: changed-scope focused 126/126; Functions 511 pass/0 fail/2 emulator-only skips; Firestore+Storage emulator 513/513 with explicit project/account; storefront 36 unit + 4 browser; protected-commerce browser 34; Firebase browser runtime 9; syntax/diff clean; root production audit 0; Functions production audit 7 accepted moderate transitive findings and 0 high/critical; security checker findings remain classified synthetic fixtures/public Firebase configuration. No deployment, provider action, browser-account action, message, invoice, or payment occurred.
