...
- Firebase email-link provider configuration, exact-recipient Secret Manager version creation, one Microsoft Graph authentication email to that recipient, and the later QuickBooks Invoice send/customer email to its exact recipient are four separate approval gates.
...
- Modify: `functions/src/providers/microsoft-graph.js`
- Modify: `functions/test/providers.test.js`
...
- Produces: `requestPilotSignInLink({email}, appCheckContext)`, a `pilot_auth_email` dispatcher, `createDigitalOrder({sku,customerName,idempotencyKey}, authContext)`, `getOrderStatus({orderHandle}, authContext)`, `verifyOrderPayment({orderId,source})`, `acceptQuickBooksWebhook({rawBody,signature})`, `reconcilePendingOrders(now)`, `getCommerceReleaseState(authContext)`, two independent default-off feature flags, and Firebase Auth plus App Check-enforced customer/admin endpoints.

- [ ] **Step 1: Add failing durable-effect repository tests**

Add `claimEffect(orderId,effect,workerId,now)`, `completeEffect(orderId,effect,workerId,claimId,result)`, `recordEffectFailure(...)`, and `recoverExpiredEffects(now)` for `invoice_create` and `invoice_send`. Also add a separately scoped pre-order repository contract for a unique `pilot_auth_email` effect keyed by the approved recipient digest and a unique pilot reservation keyed by the approved recipient/SKU digest. Each outbound claim has a unique claim ID, `claimedAt`, and five-minute `leaseExpiresAt`. Assert one concurrent claimant, immutable Invoice references, exact-claim completion, duplicate completion suppression, bounded safe error codes, and no provider payload, raw recipient, action link, or secret digest in logs/responses. A stale create lease may recover the Invoice by deterministic request ID/order reference before retry. A stale invoice-send lease is ambiguous: scheduled recovery must set `status:'manual_review'` with `lastErrorCode:'invoice_send_unknown'`, permanently close that send claim, and never call `sendInvoice()` again.

For `pilot_auth_email`, the request transaction may create only one effect for the secret-bound recipient. The dispatcher claims it, generates the link in memory, and transactionally sets `dispatchStartedAt` plus `dispatchAttemptCount:1` immediately before calling Graph. A lease that expires before `dispatchStartedAt` may be reclaimed because no email could have crossed the boundary. A confirmed or ambiguous dispatch exhausts the one-attempt limit; any failure, timeout, or crash after `dispatchStartedAt` becomes `manual_review`/`pilot_auth_email_unknown` and scheduled recovery makes zero Graph calls. A separate transaction creates the recipient/SKU reservation and order together; duplicate and parallel requests can recover that same owned order but cannot create a second order or invoice.
...
Cover both `defineBoolean` declarations defaulting false; the committed project parameter file containing exactly the two allowlisted non-secret keys with Boolean literal values; the service file value always remaining `false`; and the exact file being tracked despite the general `.env*` ignore. The initial Task 6 commit writes digital `false`, but the file-schema test must continue to pass when Task 12 deliberately changes only that value to `true`; the code-level absent-value default test remains `false`. Declare `COMMERCE_PILOT_RECIPIENT_EMAIL` with `defineSecret`, bind it only to the link-request/dispatch and digital-order functions, and prove it is absent from the project dotenv, source, Firestore payloads, logs, public responses, and Hosting manifest.

Also cover digital order denial while its flag is false; token-email mismatch and unverified-email denial; candidate/secret normalization followed by fixed-length SHA-256 and constant-time comparison; a unique recipient/SKU reservation; duplicate and parallel order submissions producing exactly one order, one `invoice_create`, and one `invoice_send`; a recovered existing invoice after create timeout; confirmed send acceptance; send failure; five-minute lease expiry; stale ambiguous send to `manual_review` with no second send; no fulfillment from create/send responses; exact evidence success; mismatched evidence to `manual_review`; and recovery without another invoice or email. Assert service orders are rejected by `createDigitalOrder()` and cannot bypass `approveInvoice`. Keep `COMMERCE_SERVICE_QBO_SEND_ENABLED` false throughout these tests. Test that `getCommerceReleaseState()` requires App Check plus `admin:true` and returns only `{digitalInvoicePilotEnabled:boolean,serviceQboSendEnabled:boolean}`.
...
Run: `npm --prefix functions test -- test/commerce/order-repository.test.js test/commerce/commerce-service.test.js test/commerce/feature-flags.test.js test/commerce/quickbooks-webhooks.test.js test/providers.test.js`
...
Declare `COMMERCE_PILOT_RECIPIENT_EMAIL=defineSecret('COMMERCE_PILOT_RECIPIENT_EMAIL')`. The exact address exists only as an approved Google Secret Manager version; do not add it to `functions/.env.the-ballers-kingdom`, any other file, command argument, log, error, response, audit payload, or test fixture. Bind it only to `requestPilotSignInLink`, the authentication-mail dispatcher, and `createDigitalOrder`. Normalize the candidate and secret consistently, SHA-256 both with a `compare\0` domain prefix to equal-length buffers, and use the Node constant-time comparison primitive on those buffers. Never compare raw variable-length strings. The value and candidate comparison digest remain process-local. The repository receives only a separate `SHA-256("binding\0" + normalizedApprovedEmail)` as the protected approved-recipient binding for the effect/reservation; it is never exposed in logs, responses, evidence, or public reads.

`requestPilotSignInLink()` is a signed-out-capable `onCall` endpoint with `enforceAppCheck:true`, strict email schema/length checks, and bounded abuse controls. It returns exactly the same generic `{status:'request_received'}` response for malformed-safe, mismatched, duplicate, disabled, and approved requests. Mismatches create no mail effect and make no Firebase Admin or Graph call. While the digital flag is false, a matching request also creates no effect or email; it may write only an admin-readable redacted `pilot_auth_request_allowed_disabled` audit event so Task 12 can verify the allowlist without sending. The public response and response shape do not reveal the decision.

For an approved match while the digital flag is true, transactionally create the single-use `pilot_auth_email` effect. A dispatcher transactionally claims that effect with a unique claim and lease, calls the current documented Firebase Admin Node method `getAuth().generateSignInWithEmailLink(approvedEmail, actionCodeSettings)`, and keeps the resulting action link in memory only. Immediately before delivery it marks `dispatchStartedAt` and the sole dispatch attempt, then invokes a new injected `sendPilotAuthLink({to,link})` method on the existing `createGraphClient` adapter, which delivers a dedicated authentication template from its configured `info@ballkingdom.com` mailbox. Firebase documents that exact Admin method and custom-delivery pattern in [Generating Email Action Links](https://firebase.google.com/docs/auth/admin/email-action-links) and [`BaseAuth.generateSignInWithEmailLink`](https://firebase.google.com/docs/reference/admin/node/firebase-admin.auth.baseauth#baseauthgeneratesigninwithemaillink). A confirmed Graph response stores only a redacted `pilot_auth_email` receipt. An exception after dispatch start, including a timeout or process crash discovered by the scheduled worker, stores `pilot_auth_email_unknown`, enters manual review, and can never dispatch again. A pre-dispatch generation failure may be retried only after lease recovery because no provider call began.

`createDigitalOrder()` first requires the digital flag, App Check, and a Firebase Auth token containing `uid`, `email`, and `email_verified:true`. It performs the same constant-time comparison between the normalized token email and the bound pilot-recipient secret, loads the server catalog item, and ignores browser amount/UID/email fields. In one Firestore transaction it creates a unique reservation keyed by the approved-recipient/SKU digest and the Task 3 order with immutable `customerUid=request.auth.uid` and normalized token email. If that reservation already names an order owned by the same UID, return that safe existing order result; any ownership inconsistency fails closed to administrator review. No invoice effect is claimable before that transaction commits. It calls `createCommerceInvoice()` with `bk-order-${orderId}` and persists `realmId`, `invoiceId`, `customerId`, and `providerOrderRef`. It then separately claims `invoice_send` and calls `sendInvoice()` with the stored Invoice ID and verified allowlisted token email. Return only:
...
The opaque `orderHandle` is routing data, not authorization. The production call path remains disabled until Task 12 approvals. The pre-order `pilot_auth_email` and later order `invoice_send` effects have different keys and receipts; neither approval or receipt can satisfy the other. A known pre-send failure may retry only within the still-owned live lease before dispatch. The scheduled worker handles expired claims: deterministic create recovery may retry after exact read-back, but an expired authentication- or invoice-send claim whose dispatch started is ambiguous and must stop in manual review rather than risk a second email.
...
The same schedule calls both authentication- and order-effect recovery first. A stale `pilot_auth_email` effect may be reclaimed only if it has no `dispatchStartedAt`; otherwise it closes as ambiguous, records `pilot_auth_email_unknown`, alerts an operator with no address/link, and makes zero Graph calls. A stale create effect performs exact order-reference/read-back recovery and resumes only if no Invoice exists. A stale invoice-send effect never dispatches: it closes the effect as ambiguous, transitions the order to `manual_review`, records `invoice_send_unknown`, and emits a redacted operator alert. Tests use an injected clock to prove every pre-expiry no-op, safe pre-dispatch auth recovery, post-dispatch auth quarantine, post-expiry create recovery, post-expiry invoice-send quarantine, and zero resend calls.

- [ ] **Step 9: Add callable/request security tests**

Assert App Check rejection for `requestPilotSignInLink`; the same generic result for mismatched and approved addresses; mismatch producing no effect/Admin/Graph call; one approved address producing one queued effect and one Graph send; duplicate/parallel request suppression; ambiguous dispatch moving to `manual_review` with no resend; and separate redacted `pilot_auth_email` versus `invoice_send` receipts. Assert signed-out and unverified-email rejection for `createDigitalOrder`, constant-time token-email allowlisting, a client-supplied UID/email being ignored, immutable `customerUid` plus unique recipient/SKU reservation creation before invoice effects, exactly one order/invoice under parallel calls, and `getOrderStatus()` requiring `request.auth.uid === order.customerUid`. Cover the correct UID, wrong UID, guessed/modified handle, deleted user/token rejection, Firebase-emulator email-link completion and reused-link denial, schema/length validation, bounded polling, abuse controls, safe public errors, and administrator enforcement for manual reconciliation/refund operations. Status responses contain no customer email or accounting identifiers. The webhook endpoint uses signature/realm verification instead of App Check and never returns entity or order details.

- [ ] **Step 10: Run and commit**

```bash
npm --prefix functions test -- test/commerce/order-repository.test.js test/commerce/commerce-service.test.js test/commerce/feature-flags.test.js test/commerce/quickbooks-webhooks.test.js test/providers.test.js
npm --prefix functions run check
git add functions/src/commerce/commerce-service.js functions/src/commerce/feature-flags.js functions/.env.the-ballers-kingdom functions/.gitignore functions/src/providers/quickbooks-webhooks.js functions/src/providers/microsoft-graph.js functions/test/providers.test.js functions/test/commerce/commerce-service.test.js functions/test/commerce/feature-flags.test.js functions/test/commerce/quickbooks-webhooks.test.js functions/src/commerce/order-repository.js functions/test/commerce/order-repository.test.js functions/src/index.js firebase.json
...
**Interfaces:**
- Consumes: the App Check-enforced `requestPilotSignInLink()`, Firebase email-link completion, `createDigitalOrder()` and owner-authorized `getOrderStatus()` endpoints from Task 6, and owner-authorized single-use fulfillment grants from Task 7.
...
Render item name, server-returned price, currency, customer name/email fields, fulfillment terms, refund/cancellation links, and separate `Email me a sign-in link` and authenticated `Send payment instructions` actions. The browser may submit the address only to the App Check-enforced `requestPilotSignInLink` callable and must render its generic `request_received` response identically; it contains no direct mail-sending Firebase API and cannot tell whether an address matched. The server alone uses Firebase Admin's documented [`generateSignInWithEmailLink`](https://firebase.google.com/docs/reference/admin/node/firebase-admin.auth.baseauth#baseauthgeneratesigninwithemaillink) and the injected Microsoft Graph sender. Complete the link with Firebase's client-side sign-in-completion operation before calling `createDigitalOrder`; after authentication, treat the verified token email as authoritative and do not let the browser substitute another UID/email. Handle a consumed, expired, modified, or reused email link as signed out and require a newly approved server request; no automatic retry is allowed after an ambiguous server dispatch. Enabling the Auth provider is not authorization to create the recipient secret or send this email, and this Graph-delivered authentication email is not the later QuickBooks invoice email. Explain before each action which system sends which message and that QuickBooks will verify payment before delivery. Never render or collect card, bank, PayPal, or Venmo credentials.
...
Confirm keyboard flow, visible focus, labels, error announcements, 390px mobile layout, identical generic authentication-request UI for allowed/mismatched/duplicate outcomes, arbitrary-address submission producing no client mail call, signed-out/wrong-user status denial, email-link replay denial, single-use download-grant replay denial, and that Functions/tests/private artifacts and both Rules files remain excluded from Hosting.
...
Use an injected Firebase Admin Auth test double plus the Firebase Auth emulator's captured out-of-band link/code to test the server-generated sign-in link, successful completion, expiry, wrong email, and replay without delivering a production authentication email. If the Functions emulator requires a secret binding, use only Firebase's documented `.secret.local` override with the synthetic `pilot@example.invalid` fixture, record its exact non-secret content/hash, and remove it before packaging; never copy, access, or approximate the production recipient. The verification record must show `production Microsoft Graph authentication emails sent: 0` and `production QuickBooks invoice emails sent: 0`; those are distinct later pilot approvals.
...
Classify synthetic test fixtures separately from real findings. Confirm `functions/.env.the-ballers-kingdom` is tracked through the narrow ignore exception, contains exactly the two non-secret Boolean parameter keys, and contains no email address, credential, customer data, or unrelated setting. Scan tracked source, Firestore fixtures, logs, Hosting manifest, and release evidence for `COMMERCE_PILOT_RECIPIENT_EMAIL` values; only the secret name and synthetic `.invalid` test address are allowed. Confirm any temporary `.secret.local` fixture is absent before packaging. Do not waive a production secret or high-severity runtime vulnerability.

- [ ] **Step 3: Verify Accounting invoice and payment journeys without live outbound effects**

Use injected mocks for the documented Invoice send operation and assert its exact current method/path/headers/response envelope without sending an email. Separately inject Firebase Admin `getAuth().generateSignInWithEmailLink()` and the existing Graph client, assert the generated link is passed only in memory to `sendPilotAuthLink`, and complete the captured synthetic link in the Auth emulator. Prove a mismatched email returns the same generic result with no send; the approved fixture produces one send; duplicate/parallel calls produce no second effect; a post-dispatch timeout becomes `manual_review` with no resend; the token email must match the allowlist; parallel order calls create exactly one order/invoice; and `pilot_auth_email` and `invoice_send` have separate redacted receipts. In an Intuit sandbox, create/read only sandbox customers, items, invoices, and Payments when supported and authorized for the test account; do not send an invoice to a real address. Cover unpaid invoice, exact paid invoice, delayed verification, partial/split/wrong-amount/wrong-realm/wrong-reference evidence, fulfillment retry, service approval, and refund-insufficient-evidence behavior. Verify each against the sandbox Accounting entity, Firestore emulator/test environment, and audit receipt. No live company, Microsoft Graph authentication email, QuickBooks invoice email, payment, or refund is authorized by Task 11.
...
firebase deploy --only functions:ballkingdom-integrations:getCommerceReleaseState --project the-ballers-kingdom --account lilpelejr12@gmail.com --dry-run
...
For the Functions dry run, use the locally verified Firebase CLI `functions:<codebase>:<export>` filter to scope only the non-secret-bound `getCommerceReleaseState` export and record the line showing it loaded `functions/.env.the-ballers-kingdom`. It must not prompt for either Boolean or `COMMERCE_PILOT_RECIPIENT_EMAIL`; abort on any prompt, any ignored project dotenv override, any key beyond the two allowlisted flags, or `COMMERCE_SERVICE_QBO_SEND_ENABLED=true`. The full codebase dry run is intentionally deferred until Task 12 creates the exact recipient secret version under its own approval, because Firebase documents that a missing `defineSecret` value is prompted during deployment and that a new version requires redeploying every bound function: [Firebase secret parameters](https://firebase.google.com/docs/functions/config-env#secret_parameters). Task 11 keeps both values false, proves the admin-only `getCommerceReleaseState` contract locally, and sends nothing.
...
- [ ] **Step 4: Establish the false/false production release state before creating the recipient secret**

From the reviewed false/false `functions/.env.the-ballers-kingdom`, first dry-run and then, under the separate deployment approval, deploy only the non-secret-bound release-state export:

```bash
firebase deploy --only functions:ballkingdom-integrations:getCommerceReleaseState --project the-ballers-kingdom --account lilpelejr12@gmail.com --dry-run
firebase deploy --only functions:ballkingdom-integrations:getCommerceReleaseState --project the-ballers-kingdom --account lilpelejr12@gmail.com
```

The locally installed CLI documents `functions:<codebase>:<export>` filtering. It must report loading the tracked project dotenv and must not prompt for either Boolean or the not-yet-created recipient secret; abort on any prompt or another dotenv source. Call the App Check- and admin-protected export and record exactly `{digitalInvoicePilotEnabled:false,serviceQboSendEnabled:false}`. This establishes the required false/false production state without deploying a function bound to a missing secret. Deploy no webhook binding or endpoint unless its independent gates pass.

- [ ] **Step 5: Create the approved recipient secret, deploy the bound Functions still false/false, and prove no-send allowlisting**

Immediately before mutation, record Brian's exact-recipient secret-version approval. Enter the exact approved address only at the hidden interactive prompt; do not put it in a shell argument, file, pipe, screenshot, log, evidence document, or source. Do not use `--force`, because secret creation must not silently deploy functions:

```bash
firebase functions:secrets:set COMMERCE_PILOT_RECIPIENT_EMAIL --project the-ballers-kingdom --account lilpelejr12@gmail.com
firebase functions:secrets:get COMMERCE_PILOT_RECIPIENT_EMAIL --project the-ballers-kingdom --account lilpelejr12@gmail.com
```

The second command records version metadata only; never run the value-access command for release evidence. With both flags still false, separately approve and run the full codebase dry run, review the bound-secret list, then deploy the codebase. Firebase documents that a new secret value does not reach bound functions until they are redeployed: [Firebase secret parameters](https://firebase.google.com/docs/functions/config-env#secret_parameters).
...
firebase deploy --only functions:ballkingdom-integrations --project the-ballers-kingdom --account lilpelejr12@gmail.com
```

Read back the two Booleans as false/false. While the digital flag is false, exercise `requestPilotSignInLink` once with a mismatched address and once with the exact approved address through an App Check-valid operator harness. Both calls must return the identical generic `{status:'request_received'}` response; protected redacted audit evidence must show mismatch denial and allowlisted-but-disabled handling, with no `pilot_auth_email` effect, Firebase Admin generation, Graph call, or outbound email. Then, under the separate Hosting approval, dry-run and deploy `hosting:public` while the flag remains false. Confirm arbitrary addresses can produce only the generic response and no send, and confirm both Rules files, secret material, Functions, tests, and private artifacts are absent from the Hosting manifest.

- [ ] **Step 6: Enable one pilot SKU and the digital flag only**
...
Review and commit that two-key non-secret file before deployment; no ignored file or deploy prompt may override it. Rerun the relevant suite, run and review the separately approved `firebase deploy --only functions:ballkingdom-integrations --project the-ballers-kingdom --account lilpelejr12@gmail.com --dry-run`, verify the CLI loads the exact file without prompting and binds the already-created recipient-secret version, then perform the approved scoped Functions deploy. Call `getCommerceReleaseState` as an App Check-authenticated administrator and record exactly `{digitalInvoicePilotEnabled:true,serviceQboSendEnabled:false}`; also prove the digital path is enabled and the service path still follows its existing Graph/PDF behavior. Before submitting the approved address, submit a mismatched address once and prove the same generic response, no new `pilot_auth_email` effect, no Firebase Admin generation, and no Graph call. The already-deployed public page cannot address or send email itself; arbitrary addresses remain inert because the server secret allowlist is authoritative.

- [ ] **Step 7: Dispatch one separately approved Graph authentication email and authenticate**

After recording a fresh approval for exactly one authentication-email dispatch and its exact recipient, call `requestPilotSignInLink` once with that address. Verify one `pilot_auth_email` effect/claim, one in-memory Firebase Admin `generateSignInWithEmailLink` result, one Graph `sendPilotAuthLink` acceptance from `info@ballkingdom.com`, receipt of the authentication email, one-time link completion, and the resulting verified-email UID. Store only redacted claim/outcome evidence; never record the address, digest, link, or Graph payload. This proves authentication only; it does not create an order, invoice, QuickBooks email, or payment. Secret creation and provider enablement did not authorize this send. A duplicate/parallel request must return the same generic response and make no second send. If the dispatch outcome is ambiguous or the lease expires after `dispatchStartedAt`, verify `manual_review`/`pilot_auth_email_unknown`, make no retry, and stop the pilot for operator review.

- [ ] **Step 8: Send one separately approved QuickBooks invoice email**

Only after recording the independent QuickBooks invoice-email approval and its exact recipient, submit the authenticated digital order once. Verify `email_verified:true`, constant-time equality between the token email and the same recipient secret, and the immutable `customerUid` plus recipient/SKU reservation in the atomic order transaction before the invoice effect. Then verify exactly one order, one server-priced QuickBooks Invoice, one stored invoice ID/order reference, one documented QuickBooks Invoice send response, and one invoice email at the exact QuickBooks-approved address. Repeat/parallel submission must recover that same owned order and create neither a second order nor invoice. The Graph authentication-email approval did not authorize this action; the `pilot_auth_email` and `invoice_send` receipts must remain separate. The response and email prove neither delivery nor payment. If the invoice send outcome is ambiguous or its five-minute claim expires, confirm scheduled recovery puts it in `manual_review` with `invoice_send_unknown` and do not resend blindly.

- [ ] **Step 9: Run one approved low-value owner payment**
...
- [ ] **Step 10: Execute an approved refund if requested**
...
- [ ] **Step 11: Record the measured result**

Update the verification document with non-secret timestamps, identifiers truncated to safe suffixes, separate approvals and outcomes for recipient-secret creation, Graph authentication email, and QuickBooks invoice email without exposing any full address, secret digest, or action link; separate redacted effect receipts; parameter-file commit/hash; protected runtime flag readbacks; scheduled-reconciliation evidence; authenticated grant expiry/replay results; rollback status; and remaining rollout gates. Commit the evidence without private customer or payment data. Keep the service flag false until a separately approved release.
...
- Firebase Admin action-link generation, constant-time Secret Manager recipient allowlisting, App Check, verified-token enforcement, one recipient/SKU reservation, five-minute authentication/invoice-effect leases, stale-send quarantine/no-resend, grant replay protection, redaction, reconciliation, and refund-review limits pass automated tests.
...
- Firebase provider configuration, exact-recipient secret-version creation, the Graph-delivered Firebase authentication email/recipient, and the later QuickBooks invoice email/recipient are four separate gates; none implies another.
- `requestPilotSignInLink` is the only public authentication-mail request surface; it returns one generic result, arbitrary addresses make no send call, one approved recipient can consume at most one `pilot_auth_email` effect, and an ambiguous dispatch is never retried.
- `createDigitalOrder` requires an allowlisted verified token and atomically permits at most one order/invoice for the approved recipient/SKU; authentication- and invoice-email receipts remain separate.
...
- Production remains fail-closed until each scoped deployment, recipient-secret creation, one Graph authentication email/exact recipient, one later QuickBooks Invoice send/customer email/exact recipient, owner payment, and optional refund receives its own explicit approval and independent verification.
