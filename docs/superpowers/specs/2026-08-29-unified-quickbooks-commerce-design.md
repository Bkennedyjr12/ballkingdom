...
The commerce adapter now implements and locally tests the documented Accounting API invoice-send operation, authoritative Invoice/Payment reads, webhook ingestion as hints, and scheduled reconciliation. A 2026-08-30 authorized read-only review verified Intuit Developer workspace `The Ballers Kingdom`, app `TBK Q.B A.I`, and its production marker. The Production Webhooks endpoint is empty and no setting changed. A separately approved read-only check at `2026-08-30T23:30:50Z` received HTTP `200` for OAuth refresh and CompanyInfo and matched `The Ballers Kingdom`, but the test intentionally did not persist the rotated refresh credential. The deployment-pinned stored credential subsequently failed; an approved reconnect completed and created new secret versions without an Accounting record mutation. Continuing health is blocked by `rotating_token_persistence_runtime_fix_unreviewed_undeployed` until a reviewed deployment and fresh safe read. Production webhook configuration remains separate, and scheduled reconciliation remains mandatory.
