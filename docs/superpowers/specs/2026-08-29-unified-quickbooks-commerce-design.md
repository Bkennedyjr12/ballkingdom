...
3. **Rules and app gates:** obtain the authoritative production Firestore and Storage Rules sources, make Java/rules-unit-testing available, and identify the owner/configurator of the production Accounting app. Missing authoritative Rules source or emulator proof blocks the pilot. App invisibility blocks webhook configuration, but does not by itself block a scheduled-reconciliation pilot when the existing Accounting OAuth connection is authoritatively verified and every other gate passes.
4. **Digital-product pilot:** after explicit approvals, deploy one low-risk SKU with only `COMMERCE_DIGITAL_INVOICE_PILOT_ENABLED=true`; keep `COMMERCE_SERVICE_QBO_SEND_ENABLED=false`. Perform one QuickBooks invoice send/customer email, one owner-controlled payment, Accounting/Firebase read-back, and protected fulfillment verification. Scheduled reconciliation is the required recovery path when webhooks are unavailable.
