...
Do not treat reconnect callback success, a new secret version, or a historical CompanyInfo response as continuing health. The local implementation now loads `QBO_REFRESH_TOKEN/versions/latest` at runtime, acquires one Firestore lease for no more than five minutes, performs one refresh, validates that Intuit returned a replacement credential, adds a Secret Manager version, and reads that exact version back before returning the access credential to the Accounting adapter. Concurrent calls in one runtime share the same pending refresh; other runtimes wait boundedly or refresh sequentially from the new latest version. Firestore stores only lease ownership/expiry and redacted version-number receipts. It never stores either credential.

If refresh returns `invalid_grant`, stop and reconnect under a new exact approval. If adding the version has an ambiguous outcome, read `versions/latest`: proceed only if its in-memory value exactly matches the replacement; otherwise record the redacted operator-review error and make no Accounting call. A definite persistence/readback failure also makes no Accounting call. Never retry a stale refresh credential blindly.

This implementation remains local/unreviewed/undeployed. Do not run another production health check or pilot action until it is reviewed and separately deployed. Deployment requires confirming the runtime identity has secret-scoped `versions.access` and `versions.add` permissions on `QBO_REFRESH_TOKEN`, plus Firestore access to the one internal lease document. After deployment, run one separately approved read-only refresh/CompanyInfo check and retain only statuses, safe version metadata, and the exact company-name match. Review old-version disablement separately against rollback and audit needs; never delete or disable versions merely to tidy the list.
