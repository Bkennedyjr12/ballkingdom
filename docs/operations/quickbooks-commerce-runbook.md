...
Do not treat reconnect callback success, a new secret version, or a historical CompanyInfo response as continuing health. The local implementation lists Secret Manager versions and selects the highest numeric version whose authoritative state is `ENABLED`; disabled and destroyed versions are never selected. It acquires one Firestore claim for no more than five minutes and, immediately before Intuit, atomically records `dispatchStartedAtMs` and `attemptCount:1`. Concurrent calls in one runtime share the same pending refresh. Another runtime may reclaim only an expired claim for which dispatch never started. A started claim is never automatically retried, even after expiry or clock skew.

Every Intuit and Secret Manager operation has a bounded deadline well below the lease. After Intuit returns, exact claim ownership is rechecked before persistence. The replacement is added and its exact version read back before Accounting access. If adding the version has an ambiguous outcome, the highest enabled version is read: proceed only if its in-memory value exactly matches the replacement. Otherwise the durable state and alert become `qbo_refresh_persistence_unknown`. A timeout becomes `qbo_refresh_timeout`. `invalid_grant`, a crash/expiry after dispatch, or any unknown post-dispatch outcome becomes `qbo_reconnect_required`. None is automatically retried; reconnect requires a new exact approval, and the successful OAuth callback explicitly resets the manual-review control. Firestore stores only redacted state, reason, attempt, timing, owner, and version-number receipts—never either credential.

This implementation remains local/unreviewed/undeployed. `QBO_REFRESH_TOKEN` is intentionally absent from `defineSecret` and every Function binding; dynamic Secret Manager IAM is the sole runtime path. Do not run another production health check or pilot action until the change is reviewed and separately deployed. Deployment requires confirming the runtime identity has secret-scoped list/get/access/add permissions on `QBO_REFRESH_TOKEN`, plus Firestore access to the internal control and redacted alert documents. After deployment, run one separately approved read-only refresh/CompanyInfo check and retain only statuses, safe version metadata, and the exact company-name match. Review old-version disablement separately against rollback and audit needs; never delete or disable versions merely to tidy the list.
...
9. Run `reconcileRefund`. It may set `refunded` only when the current documented Accounting evidence exactly binds the same realm, order reference, Invoice, Payment, currency, and amount. Missing, partial, stale, ambiguous, wrong-realm, deleted, pending, or undocumented evidence preserves `paid`/`fulfilled` and records manual review.
   When exact external evidence is reconciled, the same Firestore transaction reads the bounded pending-review ledger, verifies every amount, recomputes the exact total, requires one amount match with the deterministic review ID, resolves that review, and writes the recomputed remaining total. A retry with the same evidence is a no-op and cannot decrement twice. Missing/excess work, an invalid amount, an inflated/deflated total, an ID mismatch, more than 100 pending reviews, or multiple legacy matches creates only a redacted audit manual-review receipt; the order, reviews, and pending total remain unchanged.
