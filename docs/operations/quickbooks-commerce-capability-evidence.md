...
Firebase documents that `sendSignInLinkToEmail` requests an authentication email to the supplied address. Enabling the provider is not an email-send approval. The pilot therefore has two separate outbound email gates: first, one Firebase authentication email and its exact recipient; later, one QuickBooks Invoice send/customer email and its exact recipient. Neither approval implies the other, even if the approved address is the same. See the current [Firebase email-link authentication documentation](https://firebase.google.com/docs/auth/web/email-link-auth).
...
- Approve the exact SKU, price, QuickBooks item, tax treatment, scoped deploys, one Firebase email-link authentication email and its exact recipient, one later QuickBooks invoice send and its exact customer email recipient, one owner-controlled payment, and any refund as separate production actions.
