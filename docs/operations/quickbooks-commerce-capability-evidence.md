...
| Production Accounting OAuth health | Required post-deploy gate; not yet executed | After the inactive deployment, the operator must read the published credential binding and enabled version metadata, run a bounded refresh through the deployed coordinator, prove refresh continuity by reading the newly published binding/version metadata, then read the exact configured realm and `CompanyInfo.CompanyName='The Ballers Kingdom'`. An unchanged token reports `rotationPersisted=false`; a replacement must be exact-written, read back, and published before it may report persisted. Failure stops before any authentication email, order, QuickBooks Customer, or Invoice mutation. | [`qbo-production-accounting-health-observation.json`](evidence/qbo-production-accounting-health-observation.json); [`quickbooks-token-rotation-local-evidence.md`](quickbooks-token-rotation-local-evidence.md) |
...
published credential binding and version metadata, bounded refresh, and refresh continuity. An
unchanged token reports `rotationPersisted=false`; a replacement requires exact persistence and
published readback before `rotationPersisted=true`. The gate also requires the exact realm
...
- Keep all four commerce feature flags false in the inactive project parameter file. The separately named controlled-owner lane may be enabled only for the approved owner transaction and must be disabled again before public activation. Activation enables `COMMERCE_PUBLIC_AUTH_RESUME_ENABLED` and `COMMERCE_PUBLIC_DIGITAL_CHECKOUT_ENABLED` together; `COMMERCE_CONTROLLED_OWNER_PILOT_ENABLED` and `COMMERCE_SERVICE_QBO_SEND_ENABLED` remain false. Emergency disable turns ordering off while preserving auth/resume. Record the reviewed config commit, deploy log, and protected runtime readback.
