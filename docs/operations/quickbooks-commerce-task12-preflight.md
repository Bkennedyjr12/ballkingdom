...
6. **Production Accounting OAuth health:** an approved read-only check at `2026-08-30T23:30:50Z` loaded the four existing QBO secret values only into process memory, received HTTP `200` from the OAuth refresh POST and HTTP `200` from the Accounting CompanyInfo GET, and read back exact CompanyName `The Ballers Kingdom`. Only the statuses, Boolean results, and company name are retained; no secret, realm ID, token, authorization value, endpoint containing the realm, or provider payload is retained.
...
Durable evidence paths and checksums are indexed in [`docs/operations/evidence/README.md`](evidence/README.md). The structured Intuit app observation's canonical JSON SHA-256 is `f93c3264d76cad6065f83d1d6ef22190421633d3a80c22ad226744cb78b49937`; the production Accounting health observation's canonical JSON SHA-256 is `cec93bc108edba5819a16a9d59d341d26315d33777056f1fa82b6e96d7ad8505`.
...
5. Intuit sandbox, refund/reversal authoritative readback, supported dependency disposition, scoped Firebase dry runs/deploys, recipient secret creation, Auth provider configuration, feature-flag activation, and every outbound/pilot effect remain pending under their existing separate gates. The dated production CompanyInfo read is not sandbox or entity-write proof.
...
4. Treat the production Accounting connection as healthy only for the recorded observation time; require a fresh read-only health check if the pilot release occurs later or the OAuth connection changes.
5. After the source, artifact, dependency, and emulator blockers are resolved, request separate approvals for each production-target dry run and each scoped Rules/Functions/Hosting deployment. Keep both commerce flags false through the fail-closed release proof.
6. Treat recipient-secret creation, Firebase email-link provider configuration, one exact-recipient Graph authentication email, one QuickBooks invoice send, one owner-controlled payment, settlement verification, and any refund as distinct later approvals. The health check, app visibility, and webhook access authorize none of them.
