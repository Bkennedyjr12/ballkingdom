...
function memoryLimits(initial = {}) {
  ...
  const counts = {
    ...
    appGlobal:initial.appGlobalCount ?? 0,
  };
  return {
    calls,
    async consumePublicAuthLimits(input) {
      ...
      if (counts.email >= input.emailLimit || counts.ip >= input.ipLimit
        || counts.appGlobal >= input.appGlobalLimit) return false;
      counts.email += 1;
      counts.ip += 1;
      counts.appGlobal += 1;
      return true;
...
test('allows bounded distinct public recipients without an allowlist', async () => {
  ...
  assert.deepEqual(repository.calls[0], {
    ...
    windowMs:600000,emailLimit:5,ipLimit:20,appGlobalLimit:250,
  });
});

test('fails closed on email, IP, or independently reachable App Check app-global exhaustion', async () => {
  for (const initial of [
    {emailCount:5},{ipCount:20},{appGlobalCount:250},
  ]) {
