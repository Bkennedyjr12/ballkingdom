...
test('rejects mismatched Invoice identity, accounting, currency, and amount evidence', async t => {
  const cases = [
    ...
    ['customer reference', evidence => { evidence.invoice.customerId = 'customer-other'; }],
    ['item reference', evidence => { evidence.invoice.itemId = 'item-other'; }],
    ['tax code', evidence => { evidence.invoice.taxCode = 'TAX'; }],
    ['quantity', evidence => { evidence.invoice.quantity = 2; }],
    ['line amount', evidence => { evidence.invoice.lineAmountCents = 4800; }],
    ['unit price', evidence => { evidence.invoice.unitPriceCents = 4800; }],
