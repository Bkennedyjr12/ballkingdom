...
[[ -f "$anchor" && -f "$stock_dir/pexels-6084027.mp4" && -f "$stock_dir/pexels-7187047.mp4" && -f "$stock_dir/pexels-6077711.mp4" && -f "$narration" ]] || {
  echo 'Missing approved anchor, licensed source footage, or narration.' >&2; exit 2;
}
mkdir -p "$output_dir"
...
ffmpeg -hide_banner -y \
  ...
  -filter_complex '
    ...
    [16:a]aformat=sample_rates=48000:channel_layouts=stereo,volume=1.15[narration];
    [pulse][air][narration]amix=inputs=3:duration=first:normalize=0,alimiter=limit=0.88,afade=t=out:st=43.5:d=1.5[a]
  ' \
